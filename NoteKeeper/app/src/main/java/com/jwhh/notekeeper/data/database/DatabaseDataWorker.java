...
public class DatabaseDataWorker {
    ...
    private void insertCourse(String courseId, String title) {
        ...
        values.put(NoteKeeperDBContract.CourseInfoTable.COLUMN_COURSE_ID, courseId);
        values.put(NoteKeeperDBContract.CourseInfoTable.COLUMN_COURSE_TITLE, title);

        long newRowId = mDb.insert(NoteKeeperDBContract.CourseInfoTable.TABLE_NAME, null, values);
    }

    private void insertNote(String courseId, String title, String text) {
        ContentValues values = new ContentValues();
        values.put(NoteKeeperDBContract.NoteInfoTable.COLUMN_NOTE_TITLE, title);
        values.put(NoteKeeperDBContract.NoteInfoTable.COLUMN_NOTE_TEXT, text);
        values.put(NoteKeeperDBContract.NoteInfoTable.COLUMN_COURSE_ID, courseId);

        long newRowId = mDb.insert(NoteKeeperDBContract.NoteInfoTable.TABLE_NAME, null, values);
    }
